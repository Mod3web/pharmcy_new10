...
{
    public partial class Item_serch_storg : Form
    {
        public Item_serch_storg()
        {
            InitializeComponent();
        }
        DataTable dt;
       DataTable dt_search = new DataTable();
        ...
        private void Item_serch_storg_Load(object sender, EventArgs e)
        {
            ...
            //DataRow[] dr_Item_Storg = dt.Select("Item_no = " + vr.dgv_sals.CurrentRow.Cells[1].Value);
            ...
            dgv_multy_Item_dgv.Sort(dgv_multy_Item_dgv.Columns[0], ListSortDirection.Ascending);// ترتيب علا حسب تاريخ الصلاحية
                ...
                for (int i = 0; i < dgv_multy_Item_dgv.Rows.Count; i++)
                {
                    if (dgv_multy_Item_dgv.Rows[i].Cells[8].Value.ToString() == "0")
                    {
                        ...
                        //dgv_multy_Item.Rows[i].Visible = false;
                    }
        ...
        //    try
        //    {
        //        for (int i = 0; i < dgv_multy_Item.Rows.Count; i++)
        //        {
        //            if (checkBox2.Checked)
        //            {

        //                if (dgv_multy_Item.Rows[i].Visible == false)
        //                    dgv_multy_Item.Rows[i].Visible = true;
        //            }
        //            else
        //            {
        //                if (dgv_multy_Item.Rows[i].DefaultCellStyle.BackColor == Color.OrangeRed)
        //                {
        //                    dgv_multy_Item.Rows[i].Selected = false;
        //                    dgv_multy_Item.Rows[i].Visible = false;
        //                }
        //            }
        //        }
        ...
        void Add_Item()
        {
            ...
            if (dgv_multy_Item_dgv.CurrentRow.Cells[8].Value.ToString() != "0")
            {
                    int temp = 0;
                var vr = Application.OpenForms["Main"] as Main;
                    temp = vr.dgv_sals_dgv.RowCount;
                vr.dgv_sals_dgv.Rows.Add();
                vr.dgv_sals_dgv.Rows[vr.dgv_sals_dgv.Rows.Count - 1].Cells[1].Selected = true;
                    new Sales.Sales_open_Add().fill_Com_(vr.dgv_sals_dgv, Convert.ToInt32(dgv_multy_Item_dgv.CurrentRow.Cells[0].Value), Convert.ToInt32(dgv_multy_Item_dgv.CurrentRow.Cells[7].Value), (int)com_unit.SelectedValue);
                    if (temp != vr.dgv_sals_dgv.RowCount)
                    {
                        vr.dgv_sals_dgv.CurrentRow.Cells[1].Value = dgv_multy_Item_dgv.CurrentRow.Cells[0].Value;// الكمية
        ...
        private void Dgv_multy_Item_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            Add_Item();
        }

        private void Txt_qty_sals_TextChanged(object sender, EventArgs e)
        {
